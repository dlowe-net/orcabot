...
(defmethod examine-message ((self base-module)
  ...
  (when (member (source message) (authnicks-of self) :test #'irc-string-equal)
    (log:log-message :info "De-authenticating ~a due to quit." (source message))
    (setf (authnicks-of self)
          (delete (source message) (authnicks-of self) :test #'irc-string-equal)))
...
(defmethod examine-message ((self base-module)
  ...
  (when (string= (source message)
    ...
    (pushnew (first (arguments message)) (autojoins-of self) :test #'irc-string-equal)
    (write-to-file (data-path "autojoins.lisp") (autojoins-of self))))
...
(defmethod examine-message ((self base-module)
  ...
  (when (string= (source message) (nickname (user (connection message))))
    ...
    (setf (autojoins-of self)
          (delete (first (arguments message)) (autojoins-of self) :test #'irc-string-equal))
    (write-to-file (data-path "autojoins.lisp") (autojoins-of self))))
...
(defun access-denied (base-module module message &optional command)
  ...
  (loop
     ...
     do (when (and (or (not (member :user patterns))
                      (irc-string-equal (getf patterns :user) nick))
                  (or (not (member :auth patterns))
                      (and (irc-string-equal (getf patterns :auth) nick)
                           (member nick (authnicks-of base-module) :test 'irc-string-equal)))
                  (or (not (member :channels patterns))
                      (member (first (arguments message))
                              (getf patterns :channels)
                              :test #'irc-string-equal))
